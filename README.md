...
- [x] Prompt 04 — Release Impact + Opportunities + Action Briefs：12 个真实 release（authoritative URL）、覆盖判定（**1 充分/11 insufficient_history**，v0.2 修正窗口 bug 后重算）、44 个 Opportunities（证据门 + Investigation Priority + 确定性状态）、44 份 MiMo Action Brief（needs_refinement 宽簇守卫）。**未扩 30 天数据集**。Release Impact v0.1 存在 before/after 窗口相同 bug（`clipped_window()` 参数相同），v0.2 使用非重叠窗口修正
